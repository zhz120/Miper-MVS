...
class LSTM(nn.Module):
    ...
    def forward(self, c, h, x, bias):  # h 上一层的输入 z update r reset cr->bf cz->bi cq->bc bo
        # print('len:x_list')
        # print(len(x_list))
        ...
        # print(hx.shape)
        # TODO
        ft = torch.sigmoid(self.conv_ft(hx)+bias[0])  # bf是bias ft = forget_gate
        it = torch.sigmoid(self.conv_it(hx)+bias[1])
        c_t = torch.tanh(self.conv_c_t(hx)+bias[2])
        ct = c * ft + it * c_t
        ot = torch.sigmoid(self.conv_ot(hx)+bias[3])
        ht = ot * torch.tanh(ct)
        return ct, ht
