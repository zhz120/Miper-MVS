    ...
    def forward(self, ref_feature, src_features, ref_proj, src_projs, depth_sample, inverse_depth_min=None,
        ...
        if view_weights == None:
            ...
            for l in range(1, 4):
                ...
                i = 0
                # print('len(zip(src_features[f"level{l}"], src_projs[f"level{l}"]))')
                # print(len(src_features[f"level{l}"]))
...
class Update(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_sample):
        ...
        # self.lstm04 = LSTM(hidden_dim, input_dim)
        # self.lstm08 = LSTM(hidden_dim, input_dim)
        # self.lstm16 = LSTM(hidden_dim, input_dim)
        # TODO 这里需要三个尺度的LSTM 下面forward也要对应写好更新规则 还需要增加上下采样的方法进行多尺度融合

        self.context_zqr_convs = nn.Conv2d(hidden_dim, hidden_dim * 4, 3, padding=3 // 2)
        # TODO 三个尺度的偏置的尺度也是不同的 RAFT在特征提取器里是通过卷积进行下采样得到的 在这里也是需要对齐尺度的 所以也会有下采样 三个尺度的偏置自然就不同了
        ...
        self.hidden_bias_head = nn.Sequential(
            nn.Conv2d(num_sample, 64, 3, stride=1, padding=1, dilation=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, hidden_dim, 1, stride=1, padding=0, dilation=1),
        )
    ...
    def bias_init(self, corr):
        hidden = self.hidden_bias_head(corr)
        # hidden = F.interpolate(hidden,
        #                        scale_factor=2, mode='bilinear')
    ...
    def forward(self, h, c, normalized_depth, corr, bias, confidence=None, confidence_flag=False):
        # print('corr')
        # print(corr.shape)
        # print('hidden')
        # print(h.shape)
        ...
        h, c = self.lstm(h, c, cat, bias)
        confidence_new_0 = None
        confidence_new = None
        if confidence_flag:
            confidence_new_0 = self.confidence_head(h)
            confidence_new = torch.sigmoid(confidence_new_0)

        probability = torch.softmax(self.depth_head(h), dim=1)

        with torch.no_grad():
            index = torch.argmax(probability, dim=1, keepdim=True).type(torch.float)
            index_low = index - self.radius
            index = torch.arange(0, 2 * self.radius + 1, 1, device=h.device).view(1, 2 * self.radius + 1, 1, 1).float()
            index = index_low + index
            index = torch.clamp(index, min=0, max=self.out_num_samples - 1)
            index = index.type(torch.long)
        ...
        normalized_depth = regress_index / (self.out_num_samples - 1.0)
        return h, c, normalized_depth, probability, confidence_new, confidence_new_0
...
class IterMVS(nn.Module):
    def __init__(self, iteration, feature_dim, hidden_dim, test=False):
        ...
        self.bias_conv = nn.Conv2d(32, 32 * 4, 3, padding=3 // 2)
    ...
    def forward(self, ref_feature, src_features, ref_proj, src_projs, depth_min, depth_max):
        ...
        # print('ref_feature')
        # print(ref_feature['level2'].shape)
        # # exit()
        ...
        # print('corr')
        # print(corr.shape)  # h*w = 64*80
        # exit()
        if not self.test:
            depths["initial"].append(depth)

        h = self.update.hidden_init(corr)
        bias = self.update.bias_init(ref_feature['level2'])
        # print('bias')
        # print(bias)
        # print(bias.shape)
        bias = list(self.bias_conv(bias).split(32, dim=1))
        # print(bias[0].shape,bias[1].shape,bias[2].shape,bias[3].shape,)
        # exit()

        c = h
        normalized_depth, probability = self.update.depth_init(h)

        if not self.test:
            confidence, confidence_0 = self.update.conf_init(h)
            depth = depth_unnormalization(normalized_depth, inverse_depth_min, inverse_depth_max)
        ...
        for iter in range(self.iteration):
            ...
            if not self.test:
                ...
                # print(corr.shape)
                # exit()
                h, c, normalized_depth, probability, confidence, confidence_0 = self.update(h, c, normalized_depth,
                                                                                            corr,bias,
                ...
                if iter < self.iteration - 1:
                    h, c, normalized_depth, _, _, _ = self.update(h, c, normalized_depth, corr,bias, confidence_flag=False)
                else:
                    depth = depth_unnormalization(normalized_depth, inverse_depth_min, inverse_depth_max)
                    h, c, normalized_depth, _, confidence, _ = self.update(h, c, normalized_depth, corr,bias,
                                                                           confidence_flag=True)
                    depth_upsampled = upsample(normalized_depth, upsample_weight)
                    depth_upsampled = depth_unnormalization(depth_upsampled, inverse_depth_min, inverse_depth_max)
                    confidence_upsampled = F.interpolate(confidence,
